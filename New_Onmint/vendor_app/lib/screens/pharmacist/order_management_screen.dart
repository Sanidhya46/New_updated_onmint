...
class _OrderManagementScreenState extends State<OrderManagementScreen> {
  ...
  Future<void> _acceptOrder(String orderId) async {
    try {
      ...
      if (errorMessage.contains('expired')) {
        _showExpiredOrderDialog();
        _loadOrders(); // Refresh to remove expired orders
      } else if (errorMessage.contains('already accepted') || 
                 errorMessage.contains('another provider')) {
        ToastUtils.showError('Order already accepted by another pharmacist');
        _loadOrders(); // Refresh to remove accepted orders
      } else {
        ...
        return AlertDialog(
          ...
          title: Row(
            children: [
              ...
              const Text('Order Expired'),
            ],
          ),
          content: const Text(
            'This order has expired and is no longer available for acceptance. The order will be removed from the pending list.',
  ...
  Widget _buildOrderCard(dynamic order) {
    ...
    bool isNewOrder = order['status']?.toLowerCase() == 'requested' || order['status']?.toLowerCase() == 'pending';
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              itemCount > 0 ? '$itemCount Medicine(s)' : 'Medicine Order',
                              style: const TextStyle(
                                fontSize: 14,
                ...
                // Footer Row with Time and Price
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Row(
