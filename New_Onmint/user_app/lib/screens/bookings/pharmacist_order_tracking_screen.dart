...
class _PharmacistOrderTrackingScreenState extends State<PharmacistOrderTrackingScreen> {
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 2),
                        Text('₹${offer['amount']}.00', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF001F4D))),
                        const SizedBox(height: 2),
  ...
  Widget _buildPendingDetails() {
    ...
    double totalAmt = _booking!['price']?.toDouble() ?? 0.0;
    if (totalAmt == 0.0 && !isPrescriptionBased) {
      totalAmt = medicines.fold(0.0, (sum, med) => sum + ((med['price'] ?? 0.0) * (med['quantity'] ?? 1)));
    }
                  ...
                  return Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text('₹${((med['price'] ?? 0.0) * (med['quantity'] ?? 1)).toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF001F4D))),
                      ],
                    ),
                  );
                }).toList(),
          ...
          child: Row(
            ...
            children: [
              ...
              OutlinedButton(
                ...
                child: const Text('Change', style: TextStyle(color: Color(0xFF001F4D), fontSize: 12, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
        ),
            ...
            children: [
              ...
              if (isPrescriptionBased) ...[
                ...
                Row(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _buildSummaryCol(Icons.shopping_bag_outlined, 'Items', '${medicines.length} Medicines'),
                    _buildSummaryCol(Icons.currency_rupee, 'Amount Paid', '₹${totalAmt.toStringAsFixed(2)}'),
                    _buildSummaryCol(Icons.receipt_long_outlined, 'Order ID', '#${widget.bookingId.substring(0, 8).toUpperCase()}'),
                    _buildSummaryCol(Icons.access_time, 'Estimated Acceptance', '1 - 3 Minutes'),
                  ],
                ),
              ],
            ],
